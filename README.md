...
* To kick off a jenkins job to test a pull request, run `python test_pr.py $url_of_pr_to_test`
...
* Master Key: This will not be stored anywhere, but will be used to encrypt the other passwords entered.
* GitHub Username: GitHub credentials are required to be able to retrieve the necessary PR info via the GitHub API based on the PR URL supplied.
* GitHub Password: This can either be your regular password or a personal access token. It will be stored encrypted in .credentials/github
  ...
  * Using an API token is generally preferred as it can be more conveniently revoked in the case of it becoming compromised.
...
* Jenkins Password: This can either be your regular password or a personal access token. It will be stored encrypted in .credentials/jenkins
  ...
  * Using an API token is generally preferred as it can be more conveniently revoked in the case of it becoming compromised, and especially so on our current server as HTTPS is not yet supported, and we use Basic Authentication (so your credentials are sent obfuscated but not encrypted)

### Subsequent Invocations

Future invocations of the script will only prompt for the Master Key. If the Master Key is entered incorrectly, the system passwords will not be decrypted correctly, so you will receive a 401 error when the script attempts access. If you suspect you've entered your Master Key in wrong (or if decryption has _ever_ been successful), kill the script at this point (`ctrl-c`) and rerun it. If instead you had a typo in your Master Key the first time you entered it, you can simply re-enter your system credentials when prompted.

This may be improved in the future using authenticated encryption (which is better than the current encryption scheme, but not implemented in default Python libraries).
...
The script also requires the pycrypto library to be installed. This comes installed by default in some systems, but not in others. If you get errors saying that pycrypto is not a valid import, install it using `[sudo] pip install pycrypto` and rerun the script.
